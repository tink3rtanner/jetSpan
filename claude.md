...
- **In-app**: call `runTests()` in the browser console on isochrone.html — tests rendering at every zoom level, chunk loading, pan across 7 cities, cell integrity, color mapping, smoothness distribution, OSRM boundary, click-to-pin (~125 tests)
...
- Click-to-pin destination hexes: up to 3 pinned cells with gold outlines, persistent route lines, and comparison cards (re-click or × to unpin, ESC clears all)
- Interaction perf suite: `scripts/interaction-bench.js` + `scripts/run-interaction-bench.py` (see docs/PERFORMANCE.md)
